...
def create_new_server_real(server_name, version, mirror_url, custom_java_path):
    ...
    try:
        ...
        try:
            # 1. 버전 자릿수 안전하게 파싱 (예: 1.20.4 -> 20, 26.1.2 -> 26)
            ver_parts = version.split('.')
            is_target_version = False
            
            if len(ver_parts) >= 1:
                # 첫 번째 자릿수가 26 이상이거나, 첫 자리가 1이면서 두 번째 자리가 19 이상인 경우
            ...
            # 2. 조건 만족 시 AYA DETAIL.jar 복사 진행
            if is_target_version:
                # Details 폴더 안에 있는 새 플러그인 파일 확인
                plugin_src = os.path.join("Detail plugin", "AYAdetail-1.0-SNAPSHOT.jar")
            ...
            print(f"AYA DETAIL Auto Copy Failed: {e}") # 콘솔에만 기록
...
def update_ui_player_list(server_name):
    if current_view_server == server_name:
        ...
        # 2.Userdata 파일 스캔하여 오프라인 플레이어 추가
        try:
